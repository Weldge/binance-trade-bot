...
class BinanceClientNew(Client):
    ...
    def _process_ticker_msg(self, data_list):
        for tick_info in data_list:
            ...
            if ticker_symbol not in self.all_ticker_dict or self.all_ticker_dict[ticker_symbol]["time"] < ticker_time:
    ...
    def _process_account_msg(self, data_dict):
        account_info = {"time": data_dict["E"], "balances": []}
        balance_list = data_dict["B"]
        for item in balance_list:
            balance_info = {"asset": item["a"], "free": item["f"], "locked": item["l"]}
    ...
    def get_order(self, **params):
        ...
        if latest_file_name is None:
            self.logger.error(f"Fail load order from websocket data: {prefix}, call binance restful api.")
